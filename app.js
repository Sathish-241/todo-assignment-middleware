...
const checkUserQuery = async (request, response, next) => {
...
const checkUserBody = async (request, response, next) => {
...
//Get todos API 1
app.get("/todos/", checkUserQuery, async (request, response) => {
  const { search_q = "", category = "", priority = "", status = "" } = request;
  //    console.log(status, search_q, category, priority);
...
//Get todo API 2
app.get("/todos/:todoId/", checkUserQuery, async (request, response) => {
...
app.get("/agenda/", checkUserQuery, async (request, response) => {
  const { date } = request;
  //    console.log(date, "a");
  const getDueQuery = `SELECT 
  id,
  todo,
  category,
  priority,
  ...
  due_date AS dueDate FROM todo
    WHERE due_date='${date}';`;
  const todosArray = await db.all(getDueQuery);
...
app.post("/todos/", checkUserBody, async (request, response) => {
  const { id, todo, category, priority, status, dueDate } = request;
  const { todoId } = request.params;
  const createTodoQuery = `INSERT INTO todo
    (id,todo,priority,status,category,due_date) VALUES(${id},'${todo}','${priority}','${status}','${category}','${dueDate}')
    `;
  const addTodo = await db.run(createTodoQuery);
  response.send("Todo Successfully Added");
});

//  Update todo API 5
app.put("/todos/:todoId", checkUserBody, async (request, response) => {
  const { todo, category, status, priority, dueDate } = request;
  const { todoId } = request;
